...
import { defineConfig, type Plugin, type Rollup, build as viteBuild } from "vite";
...
function bundled(): Plugin {
  return {
    ...
    async load(id) {
      ...
      const bundle = (Array.isArray(result) ? result[0] : result) as Rollup.RollupOutput;
...
function stripVitePreload(): Plugin {
  return {
    ...
    configResolved(config) {
      const pI = config.plugins.findIndex((p) => p.name === "vite:build-import-analysis");
