...
describe("global property isolation", () => {
  test.each([
    ...
    await expect(expression(sandbox, `typeof ${property}`)).resolves.toBe("undefined");
  });
});
...
describe("network isolation (default CSP)", () => {
  test("cannot fetch external URLs", async () => {
    sandbox = await createSandbox();
    await expect(expression(sandbox, "await fetch('https://httpbin.org/get')")).rejects.toThrow();
  });
...
describe("code import isolation", () => {
  test("cannot use import statements", async () => {
    sandbox = await createSandbox();
    await expect(sandbox.run('import _ from "https://esm.sh/underscore"')).rejects.toThrow();
  });
...
describe("global scope isolation", () => {
  ...
  test("cannot access host functions not explicitly exposed", async () => {
    ...
    sandbox = await createSandbox();
    await expect(expression(sandbox, "globalThis.__hostSecret")).resolves.toBeUndefined();
...
describe("prototype and constructor attacks", () => {
  test("cannot escape via Function constructor", async () => {
    ...
    await expect(expression(sandbox, "new Function('return typeof document')()")).resolves.toBe(
      "undefined",
    );
  });
  ...
  test("cannot escape via eval", async () => {
    sandbox = await createSandbox();
    await expect(expression(sandbox, "eval('typeof document')")).resolves.toBe("undefined");
  });
  ...
  test("cannot pollute Array.prototype to affect host", async () => {
    ...
    expect(([] as unknown as Record<string, unknown>).__polluted).toBeUndefined();
  });
});
...
describe("iframe sandbox attribute", () => {
  ...
  test("iframe is hidden", async () => {
    ...
    const iframe = document.querySelector('iframe[name="hidden-test-iframe"]') as HTMLIFrameElement;
